namespace VisualNovle {
    export async function theCurse(): ƒS.SceneReturn {
        ...
        let storiesText = {
            ...
            before_the_accident: {
                Mother_001: "steh auf " + `${dataForSave.nameProtagonist}` + " wir müssen die Karotten ernten.",
                Protagonist_002: "ich komme gleich ich esse Kutz noch ein Stück Brot.",
                Mother_003: "ok ich gehe schon mal vor, komm dann nach.",
                Narrator_004: "nach dem " + `${dataForSave.nameProtagonist}` + " sein Brot gegessen hatte machte er sich auch auf dem Weg zum Feld.",
                ...
                Mother_011: "….",
                Narrator_012: "der Junge zieht die Wurzel raus, die Mutter springt zu  " + `${dataForSave.nameProtagonist}` + " und hebt in den Ohren zu."
            ...
            get_help: {
                Narrator_001: `${dataForSave.nameProtagonist}` + " geht sich zu dem Dorf Arztl, und schildert ihm die Situation,",
                Narrator_002: "Der Arzt namens Dr.Bader ging und " + `${dataForSave.nameProtagonist}` + " die gingen schnellstmöglich zum Feld zurück."
            },
            ...
            transition_to_the_village: {
                Narrator_001: "der Arzt und " + `${dataForSave.nameProtagonist}` + " bringen die Mutter vorsichtig zurück ins Dorf."
            },
            about_the_way: {
                ...
                Doctor_003: "Das einzige, das Mir bekannt wehre, ist ein Magische pflanze, die im " + `${locations.forest.name}` + "  Zu finden ist.",
                ...
                Doctor_007: "Du musst nach Norden zu den " + `${locations.grasslands.name}` + ", aber pass auf dort wimmelt es von Schleimen sie sind nicht zwar nicht stark, aber man sollte sich trotzdem von ihnen in Acht nehme.",
                Doctor_008: "nach den Felder kommst du zu dem " + `${locations.mountains.name}` + ", wenn du dich beeilst, kommst du noch bis heute Abend dort an.",
                ...
                Doctor_012: "Dahinter ist schon der " + `${locations.forest.name}` + " .Die Blume scheint tief im Wald zu wachsen. ",
                ...
                Narrator_018: `${dataForSave.nameProtagonist}` + "rennt in sein Zimmer hol seinen Rucksack.In die Küche packt etwas zu essen und trinken eine. Schnappt sich das " + `${items.sword.name}` + " was er von seinem Vater, was jetzt eher einem Doch nach der grösser ist und eilt zur Tür.",
                Doctor_019: "Warte!!",
                Doctor_020: "Nimm da hier, ein " + `${items.healing_potion.name}` + ". Er ist zwar nur schwach, aber besser als gar nicht.",
                Doctor_021: "Ich hoffe du wirst ich nicht brauchen.",
                Protagonist_022: "Danke. Passen sie auf meine Mutter auf.",
                Narrator_023: "Und so machte sich " + `${dataForSave.nameProtagonist}` + " auf ein Abenteuer."
            }
