    ...
    export async function saveMother(): ƒS.SceneReturn {
        ...
        let storyTexts: { [paragraphName: string]: StoryText } = {
            save_mother_with_flower: {
                ...
                Narrator_006: { text: `${characters.doctor.name}` + " verarbeite die Blume zu Medizin und schüttet die die Flüssigkeiten über die Regungslose Mutter." },
            ...
            save_mother_with_wather: {
                ...
                Narrator_022: { text: `${dataForSave.nameProtagonist}` + " übergibt das quell Wasser " + `${characters.doctor.name}` },
                Narrator_023: { text: `${characters.doctor.name}` + " schüttet das Wasser über die Regungslose Mutter." },
